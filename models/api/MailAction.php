        ...
        foreach ($BPs as $bp) {
            ...
            if ($isFirst) {
                foreach (arBodyPatt::find()->BPreplace($bp->Name) as $bpr) {
                    Yii::trace('Replace before: <' . $Body . '>', self::LOG_CATEGORY);
                    $Body = preg_replace((string) $bpr->Pattern, (string) $bpr->Replace, $Body);
                    Yii::trace('Replace after: <' . $Body . '>', self::LOG_CATEGORY);
                }
            }
            $isFirst = false;
